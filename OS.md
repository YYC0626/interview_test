...
11. 乐观锁？悲观锁？
回答：
    乐观锁和悲观锁是两种思想，主要用于解决并发场景下的数据竞争问题。

*   乐观锁认为在操作时数据不会被修改，所以不会对数据加；只是在执行更新的时候检查数据是否被修改了：别人修改了数据就放弃操作，否则就会执行操作。

*   悲观锁认为别人会修改数据，所以在操作数据时会把数据锁住，操作完再解锁。


    1.  乐观锁实现：
        1.  CAS （Compare And Swap）
            包括了三个操作数：需要读写内存的位置（V）,进行比较的预期值（A），拟写入的值（B）；
            逻辑为：如果位置V的值就是预期值A，我们就更新其为B，否则不进行任何操作。
            （许多CAS操作都是自旋的）
            **CAS也存在问题：一个栈，栈内发生了变化，但栈顶还是原来的栈顶，则CAS无法发现栈被修改过**
            即CAS无法发现数据被修改然后又改了回来这一操作。


        2.  版本号机制
            在数据中添加一个字段version，每次数据被更改，version都会加一；线程查询数据时同版本号一起取出，更新数据时先检查版本号是否一致，一致才进行操作。
    
优缺点和适用场景：
    乐观锁和悲观锁并无优劣之分，他们有各自的使用场景。
        1.  乐观锁适用场景比较受限。如CAS仅能保证单个变量的原子性
        2.  两者都可以使用的时候：如果竞争更激烈，选择悲观锁；竞争不太激烈，使用乐观锁。 