...
![internet message](./internet.jpeg)
...
链路层地址有几种不同的称呼：LAN地址、物理地址、MAC地址。

六个字节，采用十六进制表示，中间用短线连接：1A-23-F9-CD-06-9B



####	以太网帧格式 

前同步码8字节 + 目的地址6字节 + 源地址6字节 + 类型2字节 + 数据（46～1500）字节  + CRC 4字节
...
ARP只为同一子网内的主机或者路由器接口解析IP地址。


每个主机缓冲区都有ARP列表用于存储IP<-->MAC地址映射
带有时间戳，过期删除
